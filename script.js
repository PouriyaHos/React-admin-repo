 ...
 class App extends Component {
  ...
  render() {
    ...
    return (
      ...
      React.createElement(FilterTextField, { onTextChange: this.handleSearchChange }),
      React.createElement(Table, {
        data: data.filter(row => row.name.toLowerCase().indexOf(searchTerm) !== -1),
        data: data.filter(row => row.username.toLowerCase().indexOf(searchTerm) !== -1),
        data: data.filter(row => row.email.toLowerCase().indexOf(searchTerm) !== -1),
        columns: columns })));
  }
}
